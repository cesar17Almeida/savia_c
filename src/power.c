...
savia_wake_reason_t power_deep_sleep(const station_config_t *cfg) {
    ...
    // RP2350: powman + the same GPIO wake). On the Pico this is the energy win
    // vs the Pi Zero, which has no true deep sleep.
    //
    // For now a light-sleep stand-in so the supervisor logic runs end-to-end
    // (on desktop or before the dormant path is wired).
    const uint32_t step_ms = 50;
    uint32_t elapsed = 0;
    while (elapsed < cfg->sleep_seconds * 1000u) {
