...
// Mean of the (port, kind, depth) bucket of the hour at `hour_ms`; -1 if absent.
static float bucket_mean(uint64_t hour_ms, uint8_t port, uint8_t kind, uint8_t depth) {
    savia_aggregate_t agg[16];
    size_t n = storage_aggregate_hourly(hour_ms, hour_ms + HOUR_MS, 0, agg, 16);
    for (size_t i = 0; i < n; i++)
        if (agg[i].port == port && agg[i].kind == kind && agg[i].depth_cm == depth)
            return agg[i].mean;
    return -1.0f;
}
...
static void test_fill(void) {
    ...
    assert(bucket_mean(cur, 1, READING_SOIL_MOISTURE, 10) == 0.5f);
    assert(bucket_mean(cur, 1, READING_SOIL_MOISTURE, 30) == value(cur, CEST, 30));
    assert(bucket_mean(cur, 2, READING_SOIL_MOISTURE, 30) == 0.1f);
