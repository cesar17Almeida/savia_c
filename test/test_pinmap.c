...
int main(void) {
    ...
    cfg.lora_enabled = true;   // tx=GP4, rx=GP5 by default
    pinmap_build(&cfg, pins);
    assert(pins[4].state == SAVIA_PIN_RESERVED &&
           pins[4].reason == SAVIA_PIN_REASON_LORA_UART);
    assert(pins[5].state == SAVIA_PIN_RESERVED);
    assert(pinmap_check_assign(&cfg, 4, pio, -1) == SAVIA_PIN_ASSIGN_RESERVED);

    printf("test_pinmap: OK (caps + state + assignment validation)\n");
    return 0;
}
